...
export function usePerformanceMonitor(componentName: string, thresholds = DEFAULT_THRESHOLDS) {
  ...
  useEffect(() => {
    ...
    if (renderTime > thresholds.renderTime) {
      newWarnings.push(`Slow render detected: ${renderTime.toFixed(2)}ms (threshold: ${thresholds.renderTime}ms)`);
    }

    if (renderCountRef.current > 10) {
      newWarnings.push(`High re-render count: ${renderCountRef.current} (consider memoization)`);
    }
  ...
  const trackDataProcessing = useCallback((processingTime: number, itemsCount: number, cacheHits: number, cacheMisses: number) => {
    ...
    if (processingTime > thresholds.dataProcessingTime) {
      newWarnings.push(`Slow data processing: ${processingTime.toFixed(2)}ms (threshold: ${thresholds.dataProcessingTime}ms)`);
    }
    
    if (hitRate < thresholds.cacheHitRate) {
      newWarnings.push(`Low cache hit rate: ${hitRate.toFixed(1)}% (threshold: ${thresholds.cacheHitRate}%)`);
    }
  ...
  const trackMemoryUsage = useCallback((componentsCount: number, dataSize: number = 0) => {
  ...
  const trackTimelineMetrics = useCallback((eventsCount: number, virtualizedItems: number = 0) => {
    setMetrics(prev => ({
      ...prev,
  ...
  const getPerformanceScore = useCallback(() => {
    ...
    if (metrics.renderTime > thresholds.renderTime) {
      score -= Math.min(30, (metrics.renderTime / thresholds.renderTime - 1) * 100);
    }
    
    // Deduct points for slow processing
    if (metrics.dataProcessingTime > thresholds.dataProcessingTime) {
      score -= Math.min(25, (metrics.dataProcessingTime / thresholds.dataProcessingTime - 1) * 100);
    }
    
    // Deduct points for high memory usage
    if (metrics.approximateMemoryUsage > thresholds.memoryUsage) {
      score -= Math.min(20, (metrics.approximateMemoryUsage / thresholds.memoryUsage - 1) * 100);
    }
    
    // Deduct points for low cache hit rate
    if (metrics.cacheHitRate < thresholds.cacheHitRate) {
      score -= Math.min(15, (thresholds.cacheHitRate - metrics.cacheHitRate) / 2);
    }
  ...
  const getRecommendations = useCallback(() => {
    ...
    if (metrics.renderTime > thresholds.renderTime) {
      ...
      recommendations.push('Move expensive calculations to useMemo() or Web Workers');
    }
    
    if (metrics.reRenderCount > 10) {
      recommendations.push('Add useCallback() to event handlers to prevent re-renders');
      recommendations.push('Check if useEffect dependencies are causing unnecessary re-renders');
    }
    ...
    if (metrics.eventsDisplayed > 500 && metrics.virtualizedItemsRendered === 0) {
      recommendations.push('Implement list virtualization for better performance');
    }
  ...
  }, [componentName, metrics, warnings, getPerformanceScore, getRecommendations]);
