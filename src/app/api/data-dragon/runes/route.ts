...
export async function GET() {
  try {
    ...
    return NextResponse.json({ 
      ...
      totalRunes: runeData.reduce((total, tree) => 
        total + tree.slots.reduce((slotTotal, slot) => slotTotal + slot.runes.length, 0), 0
      )
    });
...
export async function POST(request: Request) {
  try {
    ...
    if (type === 'tree' && treeId) {
      ...
      if (!tree) {
        return NextResponse.json({ error: 'Rune tree not found' }, { status: 404 });
      }
